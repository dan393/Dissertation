...
When the input \( {x_{i}} {w_{i}} \) is significantly less than the bias \({b_{i}}\) this implies that input has little effect. But over the training period the weight will be adjusted accordingly to overcome the effect of the bias. The final sum is not bounded and thus can be largely different than the output of other nodes. Activation functions are used to contain this value between a specific range, most commonly between -1 and 1. For example, a simple activation function, binary step, solve this issue. But this cutoff function however will miss many small details and may only be used in binary classification problems. While there are many activation functions available, the most commonly used alternatives are the \textit{sigmoid} function and \textit{rectified linear unit} (ReLu) which preserve more information. 
...
%     \centering
%   \includegraphics[scale=0.05]{images/activation_binary_step.png}
%   \caption{Binary step\cite{activationfunction}}
%   \label{fig:simplenetwork}
%     \end{subfigure}
...
\begin{minipage}{0.2\linewidth}
  ...
  \includegraphics[width=\textwidth]{images/activation_binary_step.png}
  \caption{Binary step\cite{activationfunction}}
  \label{fig:simplenetwork}
...
To find the correct values for the weights and biases,  the cost function needs to be minimal. By plotting a simple version of this exponential cost function against the weight of one neuron (see fig \ref{fig:costfuntiongraph}), one could theoretically simply calculate the derivative for 0, but in reality the complexity is directly proportional to the number of neurons and will have many local minimum values.
...
Instead, the slope can be calculated and moved downwards to find the minimum value that optimises w, known as gradient descent. Large step sizes may overshoot the minimum while small ones will take a long time to converge. Thus a  good approach is to adapt this learning rate and Adam is one of the best performing algorithms.

Backpropagation uses the cost function to apply changes on the weights and biases of each neuron. Taking a multi-class classification problem, such as labeling dogs, cats, and fish, the aim is to make adjustments starting from the output layer and propagating these back to the first hidden layer. If the training example has a true value of a cat and the output of the activation function has a high probability for dog, then the aim is to decrease \( a^L(x) \) for the dog label and increase the the weight for cat label. Based on one single training example however, this would create a cat label biased in the algorithm and the values would only depend on the last training example. Instead, an average of the cost function is calculated over a significant proportion of the shuffled examples in the training set that remains computationally feasible - also called the batch size. The in depth mathematical details of the algorithm are beyond the scope of this project but the main equations including formal proofs are discussed by Nielsen 2018 \cite{nielsenneural} in chapter 2.




While this idea was mentioned as early as 1970   
During the training phase of the neural network, predictions are measured against the true values of the input and weights and biases are adjusted to improve performance in a process called backpropagation that is applied after the predictions are measured.
