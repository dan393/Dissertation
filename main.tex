...
The stroke dataset contains 43400 entries of which only 783 (1.8\%) are patients who suffered a stroke as shown in \ref{fig:stroke_countplot}. Intuitively, we can also assume that strokes will occur more often in adults and elderly (see \ref{stroke_age_distplot}).

images here

A heatmap can show less intuitive correlations for a non medialy trained viewer ():
...
\begin{figure}[h!]
\centering
  \includegraphics[scale=0.2]{images/foo.png}
  \caption{Simple cost function graph\cite{costfuntion}}
  \label{fig:costfuntiongraph}
\end{figure}
